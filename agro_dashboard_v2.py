...
def login_page():
    ...
    with col2:
        ...
        st.markdown("---")
        st.caption("Version 2.0 | Bei Problemen Admin kontaktieren")
...
def load_data():
    """Lädt Dashboard_Master_DE_v2.xlsx"""
    try:
...
if is_admin:
    ...
    master_nl_filter = st.sidebar.selectbox("Niederlassung", nl_options, key='master_nl')
else:
...
with col1:
    ...
    st.download_button(
        ...
        file_name=f'top_10_{master_nl_filter}_{pd.Timestamp.now().strftime("%Y%m%d")}.xlsx',
...
with col1:
    ...
    st.download_button(
        ...
        file_name=f'worst_10_{master_nl_filter}_{pd.Timestamp.now().strftime("%Y%m%d")}.xlsx',
        mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        use_container_width=True,
        help="🔒 Export ohne Beschreibungen, mit Wasserzeichen"
    )
