...
with col1:
    ...
    with col_btn1:
        st.download_button(label="📥 Export Alle (Excel)",
    ...
    with col_btn2:
        st.download_button(label="📥 Export Top 80/20 (Excel)",
...
with col2:
    if len(df_no_revenue_display) > 0:
        ...
        fig_no_rev.add_trace(go.Bar(y=y_labels_no_rev, x=df_chart_top10['Kosten YTD'], orientation='h',
            marker_color='#ef4444', text=df_chart_top10['Kosten YTD'].apply(lambda x: f'€{x/1000:.0f}k'),
            textposition='outside'))
        
        fig_no_rev.update_layout(height=500, xaxis_title='Kosten (€)', yaxis=dict(autorange='reversed'),
            showlegend=False, title=f"Top 10 Kostenverursacher")
...
if has_product_cols:
    ...
    if len(df_products) > 0 and '1. Product Family' in df_products.columns:
        product_family_stats = df_products.groupby('1. Product Family').agg({
            'VH-nr.': 'count', 'Kosten YTD': 'sum', 'Umsätze YTD': 'sum', 'DB YTD': 'sum'
        }).reset_index()
        ...
        st.markdown("### 🔽 Sortieren nach:")
        sort_product_mix = st.selectbox("Wähle Sortierung für Produkt-Mix:",
            ["Umsätze YTD (Höchster)", "DB YTD (Höchster Gewinn)", "Marge % (Beste)", "Anzahl (Meiste Maschinen)", "Kosten YTD (Höchste)"],
            key='sort_product_mix')
        ...
        st.download_button(label="📥 Export Produktanalyse (Excel)", data=to_excel(product_family_stats),
            file_name=f'produktanalyse_{master_nl_filter}_{pd.Timestamp.now().strftime("%Y%m%d")}.xlsx',
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        """
