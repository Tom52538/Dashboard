...
Vollständige Version mit Simple Login (ohne Google OAuth)
Alle Features aus app.py - TEIL 1
"""
...
# ============================================================================
# AUTHENTICATION (SIMPLE AUTH statt Google OAuth)
# ============================================================================
...
user_is_admin = user_role in ['superadmin', 'admin']
...
def load_and_prepare_data():
    ...
    try:
        ...
        umsatz_spalten = [col for col in df.columns if 'Umsaetze' in col]
...
if user_niederlassungen == ['alle']:
    nl_options = ['Gesamt'] + sorted(df['Niederlassung'].dropna().unique().tolist())
else:
    nl_options = ['Gesamt'] + sorted(user_niederlassungen)
...
export_columns = ['VH-nr.', 'Code', 'Omschrijving', 'Niederlassung', 
                  'Kosten YTD', 'Umsaetze YTD', 'DB YTD', 'Marge YTD %']
...
ytd_umsaetze = df_overview['Umsaetze YTD'].sum()
...
for month in months:
    monthly_data.append({
        ...
        'Umsaetze': df_monthly_base[f'Umsaetze {month}'].sum(),
...
# ============================================================================
# TEIL 2 - Fortsetzung von agro_dashboard_v4.py
# Füge diesen Code NACH Teil 1 ein (ersetze "# ENDE TEIL 1")
# ============================================================================
...
df_top = df_base.copy()
df_top_relevant = df_top[df_top['Umsaetze YTD'] >= 1000]
...
top_10_display = top_10[['VH-nr.', 'Code', 'Omschrijving', 'Kosten YTD', 'Umsaetze YTD', 'DB YTD', 'Marge YTD %']].copy()
...
with col2:
    ...
    for idx, row in top_10_display.iterrows():
        ...
        fig_top.add_annotation(
            x=row['Umsaetze YTD'], y=y_label, text=f"{row['Marge YTD %']:.1f}%",
...
if "DB YTD" in sort_worst:
    ...
    worst_10 = df_worst_relevant.nsmallest(10, 'Umsaetze YTD')

worst_10_display = worst_10[['VH-nr.', 'Code', 'Omschrijving', 'Kosten YTD', 'Umsaetze YTD', 'DB YTD', 'Marge YTD %']].copy()
...
with col1:
    ...
    worst_display['Umsaetze YTD'] = worst_display['Umsaetze YTD'].apply(lambda x: f"€ {x:,.2f}")
...
with col2:
    ...
    for idx, row in worst_10_display.iterrows():
        ...
        fig_worst.add_annotation(
            x=row['Umsaetze YTD'] if row['Umsaetze YTD'] > 0 else row['Kosten YTD'],
...
if has_product_cols:
    ...
    if len(df_products) > 0 and '1. Product Family' in df_products.columns:
        ...
        product_family_stats.columns = ['Product Family', 'Anzahl', 'Kosten YTD', 'Umsaetze YTD', 'DB YTD']
        product_family_stats['Marge %'] = (product_family_stats['DB YTD'] / product_family_stats['Umsaetze YTD'] * 100).fillna(0)
        ...
        display_products['Umsaetze YTD'] = display_products['Umsaetze YTD'].apply(lambda x: f"€ {x:,.0f}")
        ...
        if '2. Product Group' in df_products.columns:
            ...
            with col1:
                ...
                display_groups['Umsaetze YTD'] = display_groups['Umsaetze YTD'].apply(lambda x: f"€ {x:,.0f}")
            ...
            with col2:
                ...
                fig_groups.add_trace(go.Bar(
                    ...
                    x=product_group_stats['Umsaetze YTD'],
                    orientation='h',
                    marker_color='#3b82f6',
                    text=product_group_stats['Umsaetze YTD'].apply(lambda x: f'€{x/1000:.0f}k'),
...
df_no_revenue = df_base[(df_base['Kosten YTD'] > 0) & (df_base['Umsaetze YTD'] == 0)].copy()
